...
def create_argparser():
    ...
    parser.add_argument("--num_replacement", type=int, default=12)
    parser.add_argument("--pfix", type=str, required=True)
...
if __name__ == "__main__":
    ...
    path_dir = f"{args.pfix}_p{args.prompt[0].replace(' ', '_')}-{hc[:8]}_e{args.epoch:03d}_s{args.seed:06d}_r{args.num_replacement:02d}_ip{int(args.paint)}"
    ...
    args.use_lora = True
    args.lora_local_path = f'loras/{args.pfix}/checkpoint-{epoch}/lora.safetensors'
    args.control_lora_local_path = f'loras/{args.pfix}/checkpoint-{epoch}/controlnet.lora.safetensors'
