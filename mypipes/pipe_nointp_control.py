...
import sys
sys.path.append('.')
from mymodels.dit import CogVideoXTransformer3DModel
...
class MyModel(nn.Module):
    ...
    def forward(
        ...
        controlnet_hiddens = self.controlnet(
        ...
        model_output = self.transformer(
            ...
            ofs=ofs_emb,
            image_rotary_emb=image_rotary_emb,
            controlnet_hiddens=controlnet_hiddens,
            return_dict=False,
        )[0]
        return model_output
...
class CogVideoXImageToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def __init__(
        ...
        self.vae_scaling_factor_image = (self.vae.config.scaling_factor
                                         if hasattr(self, "vae") and self.vae is not None else 0.7)
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt = batch_size * [negative_prompt] if isinstance(negative_prompt,
    ...
    def prepare_latents(
        ...
        num_frames_org = num_frames
        num_frames = (num_frames - 2) // self.vae_scale_factor_temporal + 2
        shape = (
            batch_size,
            num_frames,
        ...
        if self.transformer.config.patch_size_t is not None:
            shape = shape[:1] + (shape[1] +
                                 shape[1] % self.transformer.config.patch_size_t, ) + shape[2:]
        ...
        if isinstance(generator, list):
            ...
            image_latents = [
                retrieve_latents(self.vae.encode(img.unsqueeze(0)), generator) for img in image
            ]

        image_latents = torch.cat(image_latents, dim=0).to(dtype).permute(0, 2, 1, 3,
                                                                          4)  # [B, F, C, H, W]
        ...
        mask_last = mask_last.to(device) if mask_last is not None else 1
        image_latents = torch.cat([image_latents, latent_padding], dim=1)
        # Select the first frame along the second dimension
        if self.transformer.config.patch_size_t is not None:
            first_frame = image_latents[:, :image_latents.size(1) %
    ...
    def decode_latents(self, latents: torch.Tensor) -> torch.Tensor:
        latents = latents.permute(0, 2, 1, 3,
                                  4)  # [batch_size, num_channels, num_frames, height, width]
    ...
    def _prepare_rotary_positional_embeddings(
        ...
        if p_t is None:
            ...
            grid_crops_coords = get_resize_crop_region_for_grid((grid_height, grid_width),
                                                                base_size_width, base_size_height)
    ...
    def __call__(
        ...
        image = self.video_processor.preprocess(image, height=height,
                                                width=width).to(device, dtype=prompt_embeds.dtype)
        ...
        if cat_conditions is not None:
            cat_conditions = cat_conditions.to(device, dtype=prompt_embeds.dtype)
        ...
        ofs_emb = None if self.transformer.config.ofs_embed_dim is None else latents.new_full(
            (1, ), fill_value=2.0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                latent_image_input = torch.cat([image_latents] *
                                               2) if do_classifier_free_guidance else image_latents
                # latent_image_input = torch.cat([torch.zeros_like(image_latents), image_latents]) if do_classifier_free_guidance else image_latents
                ...
                controlnet_input = torch.cat([cat_conditions] *
                                               2) if do_classifier_free_guidance else cat_conditions
                controlnet_input = torch.cat([controlnet_input, latent_image_input], dim=2)
                controlnet_hiddens = self.controlnet(
                    hidden_states=controlnet_input,
                    encoder_hidden_states=prompt_embeds,
                    timestep=timestep,
