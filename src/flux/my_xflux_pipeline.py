...
class XFluxPipeline:
    def __init__(self, model_type, device, offload: bool = False):
        ...
        if "fp8" in model_type:
            ...
            self.model = load_flow_model(model_type, device="cpu")
    ...
    def __call__(self,
        ...
        if self.controlnet_loaded:
            ...
            depth = depth**3
            depth = depth * 2 - 1
            depth = np.stack([depth] * 3)
            depth = torch.from_numpy(depth).to(torch.bfloat16).to(self.device)
            controlnet_image = Resize((width, height))(depth)[None, ...]
            # ----------------------------------------------------------------------------------- #
