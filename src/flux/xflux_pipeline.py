...
class XFluxPipeline:
    def __init__(self, model_type, device, offload: bool = False, tensor_type=torch.bfloat16):
        self.device = torch.device(device)
        self.offload = offload
        self.model_type = model_type

        self.clip = load_clip('cpu').to(dtype=tensor_type)
        self.t5 = load_t5('cpu', max_length=512).to(dtype=tensor_type)
        self.ae = load_ae(model_type, device="cpu").to(dtype=tensor_type)
        if "fp8" in model_type:
            self.model = load_flow_model_quintized(model_type, device="cpu").to(dtype=tensor_type)
        else:
            self.model = load_flow_model(model_type, device="cpu").to(dtype=tensor_type)

        if self.offload:
            # cpu_offload(self.clip, device)
            # cpu_offload(self.t5, device)
            cpu_offload(self.ae, device)
            cpu_offload(self.model, device)
        else:
            self.ae.to(self.device, dtype=tensor_type)
            self.model.to(self.device, dtype=tensor_type)
        self.clip.to(self.device, dtype=tensor_type)
        self.t5.to(self.device, dtype=tensor_type)
    ...
    def set_controlnet(self, control_type: str, local_path: str = None, repo_id: str = None, name: str = None):
        ...
        if self.offload:
            self.controlnet.cpu()
            cpu_offload(self.controlnet, self.device)
